...
void	token_heredoc(t_file **last_file, t_file *file, t_token *tokens)
{
	t_file *new_file;
...
t_token	*parse_tokens_helper(t_cmd *cmd, t_token *tokens, t_file **last_file, t_file *file)
{
	...
	{
		token_heredoc(last_file, file, tokens);
		tokens = tokens->next;
	}
	return (tokens->next);
}
...
t_cmd *parse_tokens(t_token *tokens)
{
	...
	while (tokens)
	{
		while (tokens && tokens->type != TOKEN_PIPE)
			tokens = parse_tokens_helper(cmd, tokens, &last_file, file);
		if (tokens && tokens->type == TOKEN_PIPE)
